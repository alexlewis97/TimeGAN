...
def discriminative_score_metrics (ori_data, generated_data):
  ...
  def discriminator (x, t):
    ...
    with tf.compat.v1.variable_scope("discriminator", reuse = tf.compat.v1.AUTO_REUSE) as vs:
      d_cell = tf.compat.v1.nn.rnn_cell.GRUCell(num_units=hidden_dim, activation=tf.compat.v1.nn.tanh, name = 'd_cell')
      d_outputs, d_last_states = tf.compat.v1.nn.dynamic_rnn(d_cell, x, dtype=tf.compat.v1.float32, sequence_length = t)
      y_hat_logit = tf.compat.v1.contrib.layers.fully_connected(d_last_states, 1, activation_fn=None) 
  ...
  d_loss_real = tf.compat.v1.reduce_mean(tf.compat.v1.nn.sigmoid_cross_entropy_with_logits(logits = y_logit_real, 
                                                                       labels = tf.compat.v1.ones_like(y_logit_real)))
  d_loss_fake = tf.compat.v1.reduce_mean(tf.compat.v1.nn.sigmoid_cross_entropy_with_logits(logits = y_logit_fake, 
                                                                       labels = tf.compat.v1.zeros_like(y_logit_fake)))
